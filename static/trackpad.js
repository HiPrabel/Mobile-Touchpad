...
function mouseMove(touches) {
  ...
  emitInterval = setInterval(() => {
    if (lastTouch && currentTouch) {
      ...
      let dy = currentTouch.clientY - lastTouch.clientY;
      let deltaX = currentTouch.clientX - clickStart.clientX;
      let deltaY = currentTouch.clientY - clickStart.clientY;
      if (
        !broken &&
        Date.now() - touchStartTime < settings.clickTimeThreshold &&
        Math.hypot(deltaX, deltaY) < settings.clickDistThreshold
      ) {
        dx = 0;
        dy = 0;
      } else {
...
trackpad.addEventListener("touchstart", (event) => {
  ...
  if (touches.length === 1) {
    ...
    if (distanceFromEdge > (100 - settings.scrollThreshold) / 100) {
      scroll(touches);
    } else if (clicked) {
      mouseMove(touches);
      dragging = true;
...
trackpad.addEventListener("touchend", () => {
  ...
  if (currentTouch) {
    ...
    if (
      !dragging &&
      touchDuration < settings.clickTimeThreshold &&
      Math.hypot(deltaX, deltaY) < settings.clickDistThreshold
    ) {
      clicked = true;
